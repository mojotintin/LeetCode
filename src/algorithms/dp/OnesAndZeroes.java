/*
 * Created by calabash on 2017/9/27.
 * Problem Number: 474.
...
public class OnesAndZeroes {
  ...
  public int findMaxForm(String[] strs, int m, int n) {
    ...
    for (String str : strs) {
      int numZeros = 0;
      int numOnes;
      // count the number of 0 and 1 in str
      for (int i = 0; i < str.length(); i++) {
        if (str.charAt(i) == '0') {
          numZeros += 1;
        }
      }
      numOnes = str.length() - numZeros;
      for (int i = m; i >= numZeros; i--) {
        for (int j = n; j >= numOnes; j--) {
          matrix[i][j] = Math.max(matrix[i][j], matrix[i - numZeros][j - numOnes] + 1);
        }
      }
    }
    return matrix[m][n];
